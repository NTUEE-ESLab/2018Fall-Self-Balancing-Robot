...
public class MyService extends Service{
    ...
    public void doJob(){
        ...
        if (mBluetoothAdapter == null) {
            DisplayToast("Bluetooth Devices are unable, please open bluetooth!");
            bluetoothFlag  = false;
            return;
        }

        if (!mBluetoothAdapter.isEnabled()) {
            DisplayToast("Please open bluetooth and rebuild program!");
            bluetoothFlag  = false;
            stopService();
            showToast("Please open bluetooth and rebuild program!");
            return;
        }
        showToast("Bluetooth Devices are found!");
        threadFlag = true;
    ...
    public  void connectDevice(){
        DisplayToast("Trying to connect bluetooth devices, please hang on....");
        ...
        DisplayToast("Successfully connect to bluetooth devices!");
        mBluetoothAdapter.cancelDiscovery();
        try {
            btSocket.connect();
            DisplayToast("Connected! You can start controlling it!");
            showToast("Connected! You can start controlling it!");
            bluetoothFlag = true;
        } catch (Exception e) {
            showToast("Connected failed");
            try {
                btSocket.close();
                bluetoothFlag = false;
                Log.e("connectDevice：Failed", e.getMessage());
            } catch (Exception e2) {
                DisplayToast("Connected failed, can't close socket！");
            }
        }
        ...
        if(bluetoothFlag){
            try {
                ...
                Log.e("connectDevice", "Receiving message");
            } catch (Exception e) {
                e.printStackTrace();
            ...
            try {
                ...
                Log.e("connectDevice", "Successfully");
            } catch (Exception e) {
                e.printStackTrace();
                Log.e("connectDevice", e.getMessage());
            } //绑定写接口
